<#
.SYNOPSIS
    Theme, plugins, users, bridge and REST namespaces, as JSON (the full context for an AI).
.EXAMPLE
...
$s = Get-Api '/api/plugins/wordpress/site'
$c = Get-Api '/api/plugins/wordpress/context/full'
[pscustomobject]@{ site = $c.site; theme = $s.theme; plugins = $s.plugins; users = $s.users; bridge = $s.bridge; namespaces = $s.namespaces; postTypes = $c.postTypes; taxonomies = $c.taxonomies; menus = $c.menus } | ConvertTo-Json -Depth 8
