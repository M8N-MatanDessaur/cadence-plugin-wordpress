<#
.SYNOPSIS
    Items of a post type, one row each: id, title, status, slug, link, modified, builder, word count, SEO title and description.
.EXAMPLE
    ./scripts/Get-WPItems.ps1 -Type pages -Status draft
#>
[CmdletBinding()]
param(
    [string]$Type = 'pages',
...
Get-Api "/api/plugins/wordpress/entries?type=$(Esc $Type)&q=$(Esc $Query)&status=$Status&per_page=$PerPage&page=$Page" | ConvertTo-Json -Depth 6
