 ...
 *
 *   2. Adds a single helper route /cadence/v1/builder-info/{id} that returns
 *      which builder a given post is using, its version, and a hash of its data.
 *      The Cadence plugin uses this to detect builder choice without having
...
add_action('rest_api_init', function () {
    register_rest_route('cadence/v1', '/builder-info/(?P<id>\d+)', array(
        'methods'             => 'GET',
    ...
    register_rest_route('cadence/v1', '/elementor/clear-cache/(?P<id>\d+)', array(
        'methods'             => 'POST',
        'permission_callback' => function ($req) {
            return current_user_can('edit_post', (int) $req['id']);
        },
